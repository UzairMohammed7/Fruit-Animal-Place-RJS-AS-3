...
class App extends Component {
  ...
  componentDidMount() {
    this.timerId = setInterval(this.scoreChange, 1000)
  }
  ...
  imageClick = thumbnailUrl => {
    ...
    if (imageUrl === activeImageUrl) {
      ...
      this.setState(prevState => ({
        ...
        activeImageUrl: newActiveImageUrl,
      }))
    } else {
      clearInterval(this.timerId)
      this.setState({isGameFinished: true})
  ...
  render() {
    const {activeImageUrl, category, score, time, isGameFinished} = this.state
      ...
      <div className="main-container">
        ...
        {!isGameFinished && (
          <div className="img-container">
            <img className="front-end-img" src={activeImageUrl} alt="match" />
            <ul className="category-names-container">
              {tabsList.map(eachTabId => (
                <li key={eachTabId.tabId}>
                  <button
                    type="button"
                    onClick={() => this.clickTabId(eachTabId.tabId)}
                    className={`tab-button ${
                      category === eachTabId.tabId ? 'highlight-btn' : ''
                    }`}
                  >
                    {eachTabId.displayText}
                  </button>
                </li>
              ))}
            </ul>
